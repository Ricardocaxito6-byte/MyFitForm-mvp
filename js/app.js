...
function profileErrorBox(){
  if(!state.profileError) return '';
  return `<div class="card" style="background:rgba(255,90,90,.1); border-color:rgba(255,90,90,.3); margin-bottom:14px;">
    <p style="color:#ff8a8a; font-size:13.5px;">${state.profileError}</p>
  </div>`;
}
...
async function saveProfile(){
  ...
  const weightRaw = document.getElementById('p-weight').value;
  const age = Number(ageRaw);
  const height = Number(heightRaw);
  const weight = Number(weightRaw);

  // Sem valores fixos de substituição: pedimos os dados reais, para os
  // cálculos (IMC, gordura, metabolismo, etc.) refletirem a pessoa e não
  // um perfil de exemplo repetido para toda a gente.
  if(!name){
    state.profileError = 'Introduza o seu nome.';
    render(); return;
  }
  if(!ageRaw || !isFinite(age) || age < 10 || age > 100){
    state.profileError = 'Introduza uma idade válida (entre 10 e 100 anos).';
    render(); return;
  }
  if(!heightRaw || !isFinite(height) || height < 100 || height > 250){
    state.profileError = 'Introduza uma altura válida em centímetros (entre 100 e 250).';
    render(); return;
  }
  if(!weightRaw || !isFinite(weight) || weight < 30 || weight > 300){
    state.profileError = 'Introduza um peso válido em quilogramas (entre 30 e 300).';
    render(); return;
  }
  if(!state.activity){
    state.profileError = 'Selecione o seu nível de atividade.';
    render(); return;
  }
  if(!state.experience){
    state.profileError = 'Selecione a sua experiência de treino.';
    render(); return;
  }
  ...
  state.profile.height = String(height);
...
function screenWorkoutToday(){
  ...
  const sessionActive = !!state.currentWorkoutSessionId;
  return `
  <div class="screen">
    ...
    ${sessionActive ? `<p class="muted center" style="margin-bottom:14px;">Treino em curso — marque os exercícios à medida que os for fazendo.</p>` : ''}
    <div class="stack">
      ${exercises.map((e,idx)=>`
    ...
    <div style="height:20px"></div>
    ${sessionActive
      ? `<button class="btn btn-primary" ${state.workoutFinishing?'disabled':''} onclick="finishWorkout()">${state.workoutFinishing?'A concluir…':'Concluir treino'}</button>`
      : `<button class="btn btn-primary" ${state.workoutStarting?'disabled':''} onclick="startWorkout()">${state.workoutStarting?'A começar…':'Começar treino'}</button>`
    }
  </div>`;
}
function toggleExercise(i){
  state.exercisesDone[i] = !state.exercisesDone[i];
  render();
  if(state.currentWorkoutSessionId){
    const doneNames = exercises.filter((e,idx)=>state.exercisesDone[idx]).map(e=>e.name);
    dataUpdateWorkoutProgress(state.currentWorkoutSessionId, doneNames);
  }
}
...
async function startWorkout(){
  state.exercisesDone = exercises.map(()=>false);
  if(state.user){
    state.workoutStarting = true; render();
    const { data, error } = await dataStartWorkoutSession(state.user.id);
    state.workoutStarting = false;
    if(!error && data) state.currentWorkoutSessionId = data.id;
  }
  render();
}
async function finishWorkout(){
  const doneNames = exercises.filter((e,idx)=>state.exercisesDone[idx]).map(e=>e.name);
  if(state.user && state.currentWorkoutSessionId){
    state.workoutFinishing = true; render();
    await dataFinishWorkoutSession(state.currentWorkoutSessionId, doneNames);
    const { count } = await dataGetCompletedWorkoutsCount(state.user.id);
    state.workoutsCompleted = count;
    state.workoutFinishing = false;
  } else {
    state.workoutsCompleted += 1;
  }
  state.currentWorkoutSessionId = null;
  state.exercisesDone = exercises.map(()=>false);
  nav('dashboard');
}
