...
// ============================================================
// FASE 4 — TREINO: histórico real de sessões
// ============================================================

async function dataStartWorkoutSession(userId) {
  const { data, error } = await sb
    .from("workout_sessions")
    .insert({ profile_id: userId, status: "in_progress" })
    .select()
    .single();
  return { data, error };
}

async function dataUpdateWorkoutProgress(sessionId, completedExerciseNames) {
  const { error } = await sb
    .from("workout_sessions")
    .update({ completed_exercises: completedExerciseNames })
    .eq("id", sessionId);
  return { error };
}

async function dataFinishWorkoutSession(sessionId, completedExerciseNames) {
  const { error } = await sb
    .from("workout_sessions")
    .update({
...
async function dataGetCompletedWorkoutsCount(userId) {
